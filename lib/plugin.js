...
var Plugin = function(config, app) {
        ...
        var register = function(err, services) {
            ...
            _.each(that.provides, function(toProvide) {
                if (!services[toProvide]) {
                    var err = new Error("Plugin failed to provide " + toProvide + " service. " + JSON.stringify(that));
                    that.emit("error", err);
                    deferred.reject(err);
                    return;
                }
    ...
    this.destroy = function() {
        if (that.provides.length) {
            ...
            var err = new Error("Plugins that provide services cannot be destroyed. " + JSON.stringify(that));
